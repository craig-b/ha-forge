...
export function generateTypes(data: HARegistryData, outputDir: string): TypeGenResult {
  ...
  // ---- Legacy notify services (service-only targets without entities in the state registry) ----
  const notifyServices = servicesByDomain.get('notify');
  if (notifyServices) {
    const existingNotifyEntities = new Set(entityIds.filter((id) => id.startsWith('notify.')));
    // Entity-platform services apply to notify entities, not to legacy targets
    const NOTIFY_ENTITY_SERVICES = new Set(['send_message']);

    for (const [serviceName, service] of notifyServices) {
      if (NOTIFY_ENTITY_SERVICES.has(serviceName)) continue;
      const syntheticId = `notify.${serviceName}`;
      ...
      // Add a callService overload: callService('notify', 'mobile_app_phone', { message: '...' })
      const fields: string[] = [];
      ...
      callServiceOverloads.push(
        `  callService(entity: 'notify', service: '${escapeQuotes(serviceName)}', data?: ${dataType}): Promise<${hasResponse === 'true' ? 'Record<string, unknown>' : 'null'}>;`,
      );

      entityIds.push(syntheticId);
      domainsWithEntities.add('notify');
    }
  }
  ...
  const dtsContent = [
    ...
    `declare function simulate(options: { id: string; shadows: HAEntityId; signal: import('@ha-forge/sdk').SignalGenerator }): import('@ha-forge/sdk').SimulationDefinition;`,
