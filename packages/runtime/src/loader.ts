...
export async function installGlobals(haClient?: HAApiImpl, logger?: EntityLogger): Promise<void> {
  ...
  g.sensor = sdk.sensor;
  g.binarySensor = sdk.binarySensor;
  ...
  g.button = sdk.button;
  g.siren = sdk.siren;
  g.humidifier = sdk.humidifier;
  g.valve = sdk.valve;
  g.waterHeater = sdk.waterHeater;
  g.vacuum = sdk.vacuum;
  g.lawnMower = sdk.lawnMower;
  g.alarmControlPanel = sdk.alarmControlPanel;
  g.notify = sdk.notify;
  g.update = sdk.update;
  g.image = sdk.image;
...
async function loadSingleBundle(
  ...
  for (const [, value] of Object.entries(mod)) {
    if (isScenarioDefinition(value)) continue; // source-only, skip
