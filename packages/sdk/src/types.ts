 ...
 * The two forms are mutually exclusive. Cron expressions are validated by the editor's AST analyzer.
 *
 * @example
 * ```ts
 * // Fixed interval
 ...
 *
 * The runtime registers a select entity with the mode's states as options,
 * manages enter/exit transition hooks, and enforces optional guards.
 ...
 *   transitions: {
 *     away: {
 *       enter: () => ha.callService('climate.main', 'set_hvac_mode', { hvac_mode: 'eco' }),
 ...
 * Other scripts can observe schedule changes via `ha.on('binary_sensor.<id>', ...)`.
 *
 * @example
 * ```ts
